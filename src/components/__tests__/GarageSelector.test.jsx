...
/**
 * QA — GarageSelector: the sites screen.
 *
...
const sync = vi.hoisted(() => ({
  syncGarageToSheet: vi.fn(async () => {}),
  deleteGarageFromSheet: vi.fn(async () => {}),
...
const GarageSelector = (await import('../GarageSelector')).default;
...
function setStore({ garages = [], canSync = true, customer = {}, selectedGarageId = null } = {}) {
  useAppStore.setState({
    customers: [{
      id: CUSTOMER_ID, customerId: 'acme', code: 'ACME', friendlyName: 'Acme',
      ...(canSync ? { spreadsheetId: 'sheet-1' } : {}),
      garages, config: {}, ...customer,
    }],
...
const sites = () => useAppStore.getState().customers[0].garages;
const siteNamed = (name) => sites().find((g) => g.name === name);
...
describe('add a site', () => {
  it('creates it, seeds a first level, and syncs', async () => {
    ...
    await waitFor(() => expect(sync.syncGarageToSheet).toHaveBeenCalled());
  });
  ...
  it('will not submit an empty name', async () => {
    ...
    expect(sync.syncGarageToSheet).not.toHaveBeenCalled();
  });

  it('refuses a duplicate name regardless of case, without syncing', async () => {
    const user = userEvent.setup();
    setStore({ garages: [site({ name: 'North' })] });
    render(<GarageSelector />);
...
describe('when the sync fails while adding', () => {
  it('rolls the new site back so a retry cannot duplicate it', async () => {
    const user = userEvent.setup();
    sync.syncGarageToSheet.mockRejectedValueOnce(new Error('Sheets rate limit reached'));
    render(<GarageSelector />);
  ...
  it('leaves a site added elsewhere untouched while rolling back', async () => {
    const user = userEvent.setup();
    sync.syncGarageToSheet.mockImplementationOnce(async () => {
      // A concurrent change lands mid-sync, exactly as a hydrate would.
      useAppStore.getState().setGarages((prev) => [...prev, site({ id: 99, name: 'Elsewhere' })]);
    ...
    expect(sites().map((g) => g.name)).toEqual(['Elsewhere']);
  });
});
...
describe('edit a site', () => {
  it('renames it and hands the sync the previous name', async () => {
    const user = userEvent.setup();
    setStore({ garages: [site({ name: 'North', internalName: 'North' })] });
    render(<GarageSelector />);
    ...
    // Without previousGarage a rename orphans every device row keyed to the old
    // site name — the rows are found by name, not by id.
    const call = sync.syncGarageToSheet.mock.calls.at(-1)[0];
    expect(call.previousGarage).toEqual({ internalName: 'North', name: 'North' });
    expect(call.garage.internalName).toBe('North Deck');
  });
...
describe('delete a site', () => {
  it('does nothing until the confirmation is accepted', async () => {
    ...
    expect(sites()).toHaveLength(1);
    expect(sync.deleteGarageFromSheet).not.toHaveBeenCalled();
  });

  it('removes it and passes the remaining sites to the sync', async () => {
    const user = userEvent.setup();
    setStore({ garages: [site({ id: 1, name: 'North' }), site({ id: 2, name: 'South' })] });
    render(<GarageSelector />);
    ...
    // otherGarages is what spares a display group South still uses.
    const call = sync.deleteGarageFromSheet.mock.calls.at(-1)[0];
    expect(call.garage.name).toBe('North');
    expect(call.otherGarages.map((g) => g.name)).toEqual(['South']);
  });

  it('clears the selection when the deleted site was the selected one', async () => {
    const user = userEvent.setup();
    setStore({ garages: [site({ id: 1, name: 'North' })], selectedGarageId: 1 });
    render(<GarageSelector />);

    await user.click(screen.getByRole('button', { name: 'Delete North' }));
    await confirmDelete(user);

    await waitFor(() => expect(useAppStore.getState().selectedGarageId).toBeNull());
  });

  it('says so when the sheet delete fails', async () => {
    const user = userEvent.setup();
    setStore({ garages: [site({ name: 'North' })] });
    sync.deleteGarageFromSheet.mockRejectedValueOnce(new Error('Sheets rate limit reached'));
    render(<GarageSelector />);
